...
<tr id="attr_set_{{$item->attribute_id}}"><td>{{$item->attribute_name}}</td><td>{{$item->attribute_item_name}}</td><td><a href="javascript:removeAttributeSet({{$item->attribute_id}})">Remove</a></td><input type="hidden" name="attr_set[]" value="{{$item->attribute_id}}_{{$item->attribute_item_id}}"></tr>
		...
		echo $products[0]->product_pdf;exit;
		
		if(!empty($products[0]->product_pdf)) { if(isset($products[0]->product_pdf) && ($products[0]->product_pdf != '')){ ?>
<a download href="<?php echo url('/uploads/product/'.$products[0]->product_pdf) ?>""><img id="uploaded_file" src="<?php echo url('/uploads/product/pdf.jpg') ?>" >                                     
</a>
<span class="del_btn"><a onclick="return confirm('Do you want to remove this product pdf manual?')" href="{{ url('product/delete_pdf/'.$products[0]->product_id.'_'.$products[0]->product_pdf) }}">X</a></span>
<?php }}else{ ?>

<img id="uploaded_video_1" src="<?php echo url('/uploads/product/placeholder.png') ?>" >

                                    <?php } ?>
                                        </div>
                                        <div class="col-md-3">
                                          <p>Uploaded Video</p>
      <?php if(!empty($products[0]->product_video)) { if(isset($products[0]->product_video) && ($products[0]->product_video != '')){ ?>

                                             <video controls id="uploaded_video">
                                                <source src="<?php echo url('/uploads/product/'.$products[0]->product_video) ?>" type="video/mp4">
                                                Your browser does not support the video tag.
                                              </video>
  <span class="del_btn"><a onclick="return confirm('Do you want to remove this video?')" href="{{ url('product/delete_video/'.$products[0]->product_id.'_'.$products[0]->product_video) }}">X</a></span>
                                    <?php }}else{ ?>

<img id="uploaded_video_1" src="<?php echo url('/uploads/product/video-placeholder.jpg') ?>" >

                                    <?php } ?>
                                        </div>
                                        <div class="col-md-3">
                                          <p>Product Image 1</p>
                                      <?php if(!empty($pro_images[0]->product_image)) {if(isset($pro_images[0]->product_image)){?>
<img id="uploaded_img_1" src="<?php echo url('/uploads/product/'.$pro_images[0]->product_image) ?>" > 
<?php $file = $products[0]->product_id.'_'.$pro_images[0]->pi_id.'_'.$pro_images[0]->product_image ?>
<span class="del_btn"><a onclick="return confirm('Do you want to remove product image?')" href="{{ url('product/delete_images/'.$file) }}">X</a></span>

<?php }}else{ ?>

<img id="uploaded_img_1" src="<?php echo url('/uploads/product/placeholder.png') ?>" >
								  ...
								  if(!empty($pro_images[1]->product_image)) {if(isset($pro_images[1]->product_image)){ ?>
<img id="uploaded_img_2" src="<?php echo url('/uploads/product/'.$pro_images[1]->product_image) ?>" >
<?php $file = $products[0]->product_id.'_'.$pro_images[1]->pi_id.'_'.$pro_images[1]->product_image ?>
<span class="del_btn"><a onclick="return confirm('Do you want to remove this product image?')" href="{{ url('product/delete_images/'.$file) }}">X</a></span>
<?php }}else{ ?>
...
<img id="uploaded_img_1" src="<?php echo url('/uploads/product/placeholder.png') ?>" >
                                    ...
                                    <?php } ?>
                                        ...
                                        <div class="col-md-3">
                                          <p>Product Image 3</p>
                                <?php if(!empty($pro_images[2]->product_image)) {if(isset($pro_images[2]->product_image)){ ?>
<img id="uploaded_img_3" src="<?php echo url('/uploads/product/'.$pro_images[2]->product_image) ?>" >
<?php $file = $products[0]->product_id.'_'.$pro_images[2]->pi_id.'_'.$pro_images[2]->product_image ?>
<span class="del_btn"><a onclick="return confirm('Do you want to remove this product image?')" href="{{ url('product/delete_images/'.$file) }}">X</a></span>
<?php }}else{ ?>
...
<img id="uploaded_img_1" src="<?php echo url('/uploads/product/placeholder.png') ?>" >
                                    ...
                                    <?php } ?>
                                        ...
                                        <div class="col-md-3">
                                          <p>Product Image 4</p>
                            <?php if(!empty($pro_images[3]->product_image)) {if(isset($pro_images[3]->product_image)){?>
<img id="uploaded_img_4" src="<?php echo url('/uploads/product/'.$pro_images[3]->product_image) ?>" >
<?php $file = $products[0]->product_id.'_'.$pro_images[3]->pi_id.'_'.$pro_images[3]->product_image ?>
<span class="del_btn"><a onclick="return confirm('Do you want to remove this product image?')" href="{{ url('product/delete_images/'.$file) }}">X</a></span>
...
<img id="uploaded_img_1" src="<?php echo url('/uploads/product/placeholder.png') ?>" >
							  ...
							  if(!empty($pro_images[4]->product_image)) {if(isset($pro_images[4]->product_image)){?>
<img id="uploaded_img_5" src="<?php echo url('/uploads/product/'.$pro_images[4]->product_image) ?>" >

<?php $file = $products[0]->product_id.'_'.$pro_images[4]->pi_id.'_'.$pro_images[4]->product_image ?>
<span class="del_btn"><a onclick="return confirm('Do you want to remove this product image?')" href="{{ url('product/delete_images/'.$file) }}">X</a></span>
                              ...
                              <div class="box box-info">
                                  ...
                                  <div class="form-horizontal">
                                      <div class="box-body">
                                          ...
                                          <input class='btn btn-primary' name="add_customer" type='submit' value='Edit Product Price' class='submit' />
                                          <a class='btn btn-warning' href="{{ url('/addcustomer') }}">Cancel</a>
                                          <br>
                                          <br>
...
function getAttributeItem(attr_id){
  ...
  var path="http://localhost/manager/product/getAttributeItem/"+ attr_id;
