...
class SaveorderController extends Controller{
   ...
   public function index(){
	  ...
	  //$arrValue['orders']=loadModel('medical','joinUserSaveOrder',$arrArgument);
	  $arrValue['orders']=Helper::instance()->joinUserSaveOrder($arrArgument);
	  //$arrValue['order_status']=$this->order_status;
	  $arrValue['order_status']='';
	  
	  return view('frontwebsite.saveorder',$arrValue);
   ...
   public function detail(){
	  ...
	  $url = base64_encode('index.php?controller=saveorder&function=detail');
	  $this->isLogin($url);		
	  # Build Query
	  $arrArgument['join']='INNER JOIN';
	  $arrArgument['where']=intval(trim($_GET['id']));
	  $arrValue['cart']=loadModel('medical','joinUserProductSaveOrder',$arrArgument);
	  
	  # Get Order Detail
	  $arrArgument['table']='saveorders';
	  ...
	  $arrArgument['table']='save_order_billing_detail';
	  $where=array('order_id'=>$arrValue['cart'][0]['order_id']);
	  $arrArgument['where']=$where;
	  $arrArgument['operator']='';
	  ...
	  $arrArgument['table']='save_order_shipping_detail';
	  $where=array('order_id'=>$arrValue['cart'][0]['order_id']);
	  $arrArgument['where']=$where;
	  $arrArgument['operator']='';
	  ...
	  loadView('header.php');
	  loadView('save-order-detail.php', $arrValue);
	  loadView('footer.php');	   
   }
   ...
   public function reorder()
   {
	   if(isset($_GET['id']))
	   {			
			$arrArgument['table']='save_order_detail';
