  ...
  <div class="row main-contant">
    ...
    <div class="container contant-con">
      ...
      <div class="content">
		...
		/*echo '1='.session()->get('Transaction_Status');
		echo '2='.session()->get('Transaction_Message');
		echo '3='.session()->get('Transaction_ID');
		echo '4='.session()->get('AVS_Response');
		echo '5='.session()->get('CVS_Response');
		echo '6='.session()->get('Masked_Account');
		echo '7='.session()->get('Card_Type');
		echo '8='.session()->get('Authorization_Code');
		exit;*/	
				
		/*if(session()->get('Transaction Status') != null){
			echo "<span style='color:red'>Transaction Status</span>: ".trim(session()->get('Transaction Status')).'<br>';
			session()->pull('Transaction Status');
		}
		
		if(session()->get('Transaction Message') != null){
			echo "<span style='color:red'>Transaction Message</span>: ".trim(session()->get('Transaction Message')).'<br>';
			session()->pull('Transaction Message');
		}
		
		if(session()->get('Transaction ID') != null){
			echo "<span style='color:red'>Transaction ID</span>: ".trim(session()->get('Transaction ID')).'<br>';
			session()->pull('Transaction ID');
		}
		
		if(session()->get('AVS Response') != null){
			echo "<span style='color:red'>AVS Response</span>: ".trim(session()->get('AVS Response')).'<br>';
			session()->pull('AVS Response');
		}
		
		if(session()->get('CVS Response') != null){
			echo "<span style='color:red'>CVS Response</span>: ".trim(session()->get('CVS Response')).'<br>';
			session()->pull('CVS Response');
		}
		
		if(session()->get('Masked Account') != null){
			echo "<span style='color:red'>Masked Account</span>: ".trim(session()->get('Masked Account')).'<br>';
			session()->pull('Masked Account');
		}
		
		if(session()->get('Card Type') != null){
			echo "<span style='color:red'>Card Type</span>: ".trim(session()->get('Card Type')).'<br>';
			session()->pull('Card Type');
		}
		
		if(session()->get('Authorization Code') != null){
			echo "<span style='color:red'>Authorization Code</span>: ".trim(session()->get('Authorization Code')).'<br>';
			session()->pull('Authorization Code');
		}*/
