	...
	<td>
		<div class="dropdown">
			...
			<ul class="dropdown-menu"  style="right: 0; left: auto;"  aria-labelledby="dropdownMenu">
				<li class="">
					<a href="customerupdate/{{ $user->user_id }}" class="tip" data-placement="left" data-element="user38" title="View/Edit {{ $user->user_fname }} {{ $user->user_lname }}"><i class="fa fa-pencil text text-primary"></i> View/Edit {{ $user->user_fname }} {{ $user->user_lname }}</a>
				</li>

				<li role="separator" class="divider"></li>

				<li class="">
					<a href="destroycustomer/{{ $user->user_id }}" class="dAction tip" data-id="{{ $user->user_id }}" data-option="" data-type="deactivate-user" data-modal-title="De/Activate Thandi Ngobese's Account" data-modal-text="Are you sure you want to De/Activate this User Account ?" data-form="adminUsers"  data-placement="left" data-element="user{{ $user->user_id }}" title="De/Activate {{ $user->user_fname }} {{ $user->user_lname }}" ><i class="fa fa-ban text text-danger"></i> De/Activate {{ $user->user_fname }} {{ $user->user_lname }}</a>
				</li>
